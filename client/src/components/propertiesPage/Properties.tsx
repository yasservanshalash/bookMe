...
const Properties = () => {
  ...
  return (
    <Box sx={{width: "70%", height: "90vh", overflowY: "scroll", margin: "0 auto"}}>
        <Box>
        <Typography>{searchTerm === "" ? "" : `${properties.filter((property) => property.title.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()) || property.address.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())).length } properties found`}</Typography>
